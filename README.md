...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sidebar { width: 280px; background: white; box-shadow: 2px 0 10px rgba(0, 0, 0, 0.1); display: flex; flex-direction: column; position: fixed; height: 100vh; left: 0; top: 0; z-index: 100; transition: transform 0.3s ease; }
        ...
        .toggle-sidebar-btn { position: fixed; left: 290px; top: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border: none; color: white; width: 48px; height: 48px; border-radius: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 20px; z-index: 101; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.4); }
        .toggle-sidebar-btn:hover { transform: translateY(-2px) scale(1.05); box-shadow: 0 6px 20px rgba(102, 126, 234, 0.5); }
        .toggle-sidebar-btn.sidebar-hidden { left: 10px; background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); box-shadow: 0 4px 15px rgba(17, 153, 142, 0.4); }
        .toggle-sidebar-btn.sidebar-hidden:hover { box-shadow: 0 6px 20px rgba(17, 153, 142, 0.5); }
...
function bootstrapEmbedIfAny() {
	try {
		...
		if (qp.cat && qp.folder && qp.list) {
			// Par défaut: vue "study" (révision), sinon gérer "manage" ou "lists"
			if (qp.view === 'manage') {
			...
			// Pas assez d’infos → accueil minimal
			showPage('home');
		}
	} catch(e) {
...
function loadData() { 
	...
	if (Object.keys(categories).length === 0) { 
		categories = {'📚 Mes cours': {'Droit Administratif': {'Chapitre 1': [{question: "Qu'est-ce que le principe de légalité ?", answer: "Le principe selon lequel l'administration doit agir conformément aux règles de droit."}]}}}; 
...
function loadManageCardsPage() {
	...
	cards.forEach((card, idx) => {
		...
		// IMPORTANT: une seule instruction "div.innerHTML = ... ;"
		div.innerHTML =
		  '<div class="card-edit-header">' +
		    '<div class="card-edit-number">Carte #' + (idx + 1) + '</div>' +
		    '<div class="card-edit-actions">' +
		      '<button class="btn-edit-card" onclick="startEditCard(' + idx + ')">✏️</button>' +
		  ...
		  '</div>' +
		  // Éditeur inline masqué par défaut
		  '<div class="card-inline-editor" id="editor-' + idx + '" style="display:none">' +
		    '<div class="editor-row">' +
		      ...
		      '<div class="editor-toolbar">' +
		        ...
		        '<button class="editor-btn" onclick="insertImageExisting(' + idx + ', \'Q\')">🖼️</button>' +
		        '<button class="editor-btn" onclick="clearFormatExisting(' + idx + ', \'Q\')">✖</button>' +
		      '</div>' +
		      '<div class="editor-content" id="editQ-' + idx + '" contenteditable="true"></div>' +
		    '</div>' +
		    '<div class="editor-row">' +
		      '<label class="card-edit-label" style="display:block;margin-bottom:6px">Réponse / Verso :</label>' +
		      '<div class="editor-toolbar">' +
		        ...
		        '<button class="editor-btn" onclick="insertImageExisting(' + idx + ', \'A\')">🖼️</button>' +
		        '<button class="editor-btn" onclick="clearFormatExisting(' + idx + ', \'A\')">✖</button>' +
		      '</div>' +
...
function editCurrentCard() {
	if (!currentCategory || !currentFolder || !currentList) { alert("Aucune liste sélectionnée."); return; }
	const cards = categories[currentCategory][currentFolder][currentList] || [];
	if (cards.length === 0) { alert("Aucune carte dans cette liste."); return; }
	
	let idx;
	if (isReviewMode) {
		if (reviewIndices.length === 0) { alert("Aucune carte en mode révision."); return; }
...
function showEndScreen() { 
	...
	drawPieChart(); 
	const toReview = getCardsToReview().length; 
	const btnReview = document.getElementById('btnReviewWrong'); 
	if (toReview > 0) { 
		btnReview.disabled = false; 
		btnReview.textContent = '🔄 Réviser (' + toReview + ')'; 
	} else { 
		btnReview.disabled = true; 
		btnReview.textContent = '🔄 Aucune'; 
	} 
	const nextList = getNextList(); 
	const btnNext = document.getElementById('btnNextList'); 
	if (nextList) { 
		btnNext.disabled = false; 
		btnNext.textContent = 'Liste suivante : ' + nextList + ' →'; 
	} else { 
		btnNext.disabled = true; 
		btnNext.textContent = 'Dernière liste ✓'; 
	} 
}
...
function startEditCard(idx) {
	...
	// Masquer tous les éditeurs d'abord (pour n'en montrer qu'un seul à la fois)
	...
	if (panel) {
		...
		q.focus(); // Mettre le focus sur le champ question
	}
}
...
function saveEditedCard(idx) {
	...
	saveData();
	updateCategoryList();
	// Recharger la liste des cartes pour afficher la mise à jour et masquer l'éditeur
	loadManageCardsPage();
}
...
// Le code de l'écouteur imageInput a été corrigé plus haut pour gérer les deux cas.
...
function baseAppUrl() {
	// Reconstruit l’URL de l’app sans query (pour éviter d’empiler des params)
	const u = new URL(window.location.href);
	u.search = ''; // on repart propre
	u.hash = '';
	return u.toString();
}
...
function buildEmbedUrl({cat, folder, list, view}) {
	...
	if (view)   u.searchParams.set('view', view); // 'study' | 'manage' | 'lists'
	return u.toString();
}
...
function exportAll() {
	...
	// Construire un snapshot complet et lisible
	const snapshot = {
		exportVersion: "1.0",
...
function normalizeSpaces(s) { 
	// Remplacement des sauts de ligne par un espace, sauf pour les vrais double sauts de ligne
	// On veut conserver la structure du texte (paragraphes) mais normaliser l'affichage
	return s.replace(/\r\n|\r/g, '\n').replace(/([^\n])\n/g, '$1 ').replace(/\t/g, ' '); 
}
...
function showDeletionGhostAfter(anchorEl, id, removedText) {
    ...
    // Utiliser la version normalisée pour l'affichage du fantôme, mais garder les sauts de ligne si présents pour la lisibilité
...
function computeComp() {
    ...
    // Utilisation de diffWordsWithSpace pour une meilleure gestion des espaces/sauts de ligne
    ...
    for (let i=0; i<parts.length; i++) {
		...
		// 1. Gérer les remplacements (removed + added consécutifs)
        if (p.removed && !p.added) {
            ...
            if (next && next.added && !next.removed) {
                ...
                if (!isOnlySpaces(next.value) && !isOnlySpaces(p.value) && !isWhitespaceOnly) {
                    // C'est un vrai remplacement de contenu
                    const anchorId = `chg-${++idxChange}`;
					
					// Afficher la suppression barrée, suivie de l'insertion
                    ...
                    // Remplacement de style ou d'espaces/sauts de ligne non significatif. On l'ignore dans le résultat
                    // MAIS on le compte comme un remplacement s'il est techniquement différent (pour le décompte)
					if (isWhitespaceOnly) {
						// Ignorer la paire et passer l'itération
						html += escapeHtmlComp(normalizeSpaces(next.value)); // Afficher le texte non modifié
						rep++; // On compte un remplacement s'il est dans la liste de changements
            ...
            // 2. Si non remplacé (ou si le remplacement a été ignoré car blanc/stylistique dans le 'else' ci-dessus, il n'arrive pas ici)
			if (isOnlySpaces(p.value)) continue; // Si la suppression n'est que des blancs, on l'ignore
			
            const anchorId = `chg-${++idxChange}`;
            // On utilise un <a> vide comme ancre pour les suppressions, car <mark> a un style de ligne barrée que l'on veut conserver.
		...
		// 3. Gérer les insertions (ajoutées seules, non précédées d'une suppression pour un remplacement)
	...
	// Remplacer les sauts de ligne non normalisés par <br> ou <p>
	// Pour l'affichage dans le div, on remplace \n par un <br>
    outComp.innerHTML = html.replace(/\n/g, '<br>');

	
	// Mettre à jour la liste des changements en filtrant les changements d'espaces/sauts de ligne
	const finalChanges = changes.filter(c => !c.isWhitespaceOnly);
	
    renderChangesListComp(finalChanges);
	
	// Afficher les totaux réels (sans changements d'espaces/sauts de ligne ignorés)
...
function renderChangesListComp(changes) {
	...
	// 1. Appliquer le filtre de suppression (si actif)
    let filtered = changes;
	if (showOnlyDeletions) {
	    filtered = changes.filter(c => c.type === 'suppression' || c.type === 'remplacement'); // Les remplacements contiennent une suppression
	}
    ...
    for (const c of filtered) {
        ...
        if (ctxSpan > 0) {
			...
			// Pour la recherche dans le contexte, on utilise le texte brut sans normalisation des espaces pour un meilleur match
            ...
            if (pos>=0) {
				...
				// Remplacer les sauts de ligne par un petit symbole (↵) et normaliser les espaces pour l'affichage du contexte
...
document.getElementById("toggleContext").addEventListener('click', () => {
    ...
    computeComp(); // recalculera la liste des changements avec/sans contexte
    // La fonction computeComp() efface le focus, mais nous voulons le restaurer si possible après le re-render
    if (prevSelectedId) {
...
onlyDelBtn.addEventListener('click', () => {
	...
	// On re-render la liste avec le filtre
	const prevSelectedId = lastSelectedBtn ? lastSelectedBtn.dataset.id : null;
	const prevFocused = lastFocusedId;
	// On ne recalcule pas le diff, juste on rerend la liste depuis les dernières "changes"
	// Comme computeComp() encapsule la construction des "changes",
	// on appelle computeComp() pour rester simple et cohérent:
	computeComp();
	
	// Restaurer sélection/focus si possible
	if (prevSelectedId) {
